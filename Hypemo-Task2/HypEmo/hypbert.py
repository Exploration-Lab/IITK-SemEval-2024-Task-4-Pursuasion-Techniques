...
class HypBert(nn.Module):
    def __init__(self, num_labels, alpha, gamma):
        ...
        self.pooler_fc = nn.Linear(self.config.hidden_size + 512, self.config.hidden_size)
        self.pooler_fc = nn.Linear(self.config.hidden_size, self.config.hidden_size)
        self.classifier = nn.Linear(self.config.hidden_size, self.num_labels)
        # self.init_weights()
        
    def get_emedding(self, features,image_tensor):
        x = features[:, 0, :]
        x = self.pooler_fc(torch.cat([x,image_tensor],dim=1))
        x = self.pooler_fc(x)
        x = self.dropout(x)
        x = F.relu(x)
        return x
    ...
    def forward(
        ...
        if ENCODER_TYPE == 'bert-base-uncased':
            ...
            pooled_output   = self.get_emedding(outputs.hidden_states[-1],image_tensor)
