...
class HypEmo():
    ...
    #             softmax_logits = softmax(logits.cpu(), axis=1)
    #             if valid_pred is None:
    #                 valid_pred = prediction
    #                 softmax_tensor = torch.tensor(softmax_logits)
    #             else:
    #                 valid_pred = torch.cat([valid_pred, prediction])
    #                 softmax_tensor = torch.cat([softmax_tensor, torch.tensor(softmax_logits)])
    #             valid_label.extend(label.tolist())

    #     valid_pred = valid_pred.detach().cpu().numpy()
    #     softmax_numpy = softmax_tensor.detach().cpu().numpy()
    #     print("Saving.....")
    #     np.save(f"softmax_{ith_epoch}_valid.npy",softmax_numpy)
    ...
    def test_step(self, ith_epoch):
        ...
        softmax_tensor = None
        with torch.no_grad():
            for x, label,image in self.test_loader:
#                 print(x[0].shape)
                input_ids, attention_mask = x['input_ids'].to(args.device), x['attention_mask'].to(args.device)
                logits = self.model(input_ids=input_ids, attention_mask=attention_mask,image_tensor = image)['logits']
                prediction = torch.argmax(logits, dim=-1)
                softmax_logits = softmax(logits.cpu(), axis=1)
        ...
        softmax_numpy = softmax_tensor.detach().cpu().numpy()
        print("Saving.....")
        np.save(f"softmax_{ith_epoch}_test_v1.npy",softmax_numpy)
