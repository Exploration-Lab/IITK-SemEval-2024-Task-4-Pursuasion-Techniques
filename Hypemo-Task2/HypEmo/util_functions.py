...
class HyoEmoDataSet(Dataset):
    def __init__(self, dataset, mode):
        ...
        tensor = torch.load(f'./data/{dataset}/{mode}.pt')
        self.image = tensor[:][0]
        self.text = df.aug_text
        self.label = df.label
    ...
    def __getitem__(self, idx):
        return self.text[idx], self.label[idx] , self.image[idx]
    
    def collate(self, batch):
        text = [t for t,_,_ in batch]
        encode = self.tkr(text, padding='longest', truncation= True, max_length=200, return_tensors='pt')
        label = [l for _,l,_ in batch]
        label_tensor = torch.tensor(label)
        image = [i for _,_,i in batch]
        image_tensor = torch.stack(image)
        return encode,label_tensor,image_tensor
